...
int convUtf32p8(FILE* arquivo_entrada, FILE* arquivo_saida)
{
    ...
    if (fread(bom, 1, 4, arquivo_entrada) != 4) {
        fprintf(stderr, "Erro na leitura do BOM.\n");
        return -1;
    }

    // Verifica se o BOM é válido (Little-endian ou Big-endian)
    int little_endian = 0;
    if (bom[0] == 0xFF && bom[1] == 0xFE && bom[2] == 0x00 && bom[3] == 0x00) {
        little_endian = 1; // UTF-32 little-endian
    } else if (bom[0] == 0x00 && bom[1] == 0x00 && bom[2] == 0xFE && bom[3] == 0xFF) {
        little_endian = 0; // UTF-32 big-endian
    } else {
    ...
    while (fread(&utf32_char, sizeof(unsigned int), 1, arquivo_entrada) == 1) {
        if (!little_endian) {
            // Se big-endian, inverter os bytes
            utf32_char = (utf32_char >> 24) | 
                         ((utf32_char >> 8) & 0x0000FF00) |
                         ((utf32_char << 8) & 0x00FF0000) | 
                         (utf32_char << 24);
        }

        // Conversão de UTF-32 para UTF-8
        if (utf32_char <= 0x7F) {
            // 1 byte: 0xxxxxxx
            unsigned char utf8_char = utf32_char & 0x7F;
            fwrite(&utf8_char, sizeof(unsigned char), 1, arquivo_saida);
            ...
            fwrite(utf8_char, sizeof(unsigned char), 2, arquivo_saida);
            ...
            fwrite(utf8_char, sizeof(unsigned char), 3, arquivo_saida);
            ...
            fwrite(utf8_char, sizeof(unsigned char), 4, arquivo_saida);
        } else {
